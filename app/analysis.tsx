...
import { AnalyzedIngredient, OverallRating, IngredientColor } from '@/types';
...
const stats = useMemo(() => {
  ...
  const unknown = analysis.ingredients.filter(i => i.color === 'gray').length;
  ...
  <>
    ...
    >
      <View style={[styles.ratingCard, { backgroundColor: rating.bg, borderColor: rating.border }]}>
        {rating.icon}
        <Text style={[styles.ratingLabel, { color: rating.color }]}>{analysis.overallText}</Text>
